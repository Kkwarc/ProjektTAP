...
disp("\[\begin{aligned}")
disp("G_{a}(s) &=")
num = tf_model.Numerator(1, 1);
num = num{1, 1};
den = tf_model.Denominator(1, 1);
den = den{1, 1};
...
disp("\\ \\")
disp("G_{b}(z) &=")
num = tf_model.Numerator(2, 1);
num = num{1, 1};
den = tf_model.Denominator(2, 1);
den = den{1, 1};
TF=tf(num,den,1,'variable','z');
fracparts=regexp(evalc('TF'),'([^\n]*)\n[ ]*-[-]+[ ]*\n([^\n]*)','tokens');
TFlatex=['\frac{' fracparts{1}{1} '}{' fracparts{1}{2} '}'];
...
disp("\\ \\")
disp("K_{1} &=")
num = tf_model.Numerator(1, 7);
num = num{1, 1};
den = tf_model.Denominator(1, 7);
den = den{1, 1};
disp(latex(vpa(num(1)/den(1),4)))
disp("\\ \\")
disp("K_{2} &=")
num = tf_model.Numerator(2, 7);
num = num{1, 1};
den = tf_model.Denominator(2, 7);
den = den{1, 1};
disp(latex(vpa(num(1)/den(1),4)))
disp("\end{aligned}\]")
% Heavy wizardry end.
